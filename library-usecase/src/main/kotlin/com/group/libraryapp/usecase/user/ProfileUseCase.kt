...
class ProfileUseCase(
    ...
    fun get(userId: Long): UserDetailDto {
        val user = userRepository.findById(userId) ?: userNotFoundFail(userId)
        return UserDetailDto.of(user)
    }

    @Transactional
    fun update(cmd: UpdateUserCommand) {
        val user = userRepository.findById(cmd.id) ?: userNotFoundFail(cmd.id)
